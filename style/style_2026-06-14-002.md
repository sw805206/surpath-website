...
## Partner Cards (Partners page — partners.html)
The Partners page (partners.html) reuses the shared nav, hero (standard order: slogan → H1 →
body, plus a lime `.btn .btn-lime` "Contact us" CTA in `.hero-cta`), `.section.section-sage`,
`.container`, and footer. Its one new pattern is the partner-card grid. All CSS lives in
style.css under `/* Partner page */`.

### Card grid layout
- `.partner-rows`: flex column, gap 16px — stacks the two card rows with a 16px gap between them.
- `.partner-row`: CSS grid, gap 16px (gap between cards within a row). Two width variants:
  - `.partner-row--2`: `grid-template-columns: 1fr 1fr` (Row 1 — 2 equal cards).
  - `.partner-row--3`: `grid-template-columns: 1fr 1fr 1fr` (Row 2 — 3 equal cards).
- Mobile (≤768px): both `--2` and `--3` collapse to a single column.

### Card
- `.partner-card`: flex row, white bg, 0.5px #d4e8c8 (card border) border, 8px border-radius,
  overflow hidden. Content side drives card height (no fixed card/column height).
- `.partner-card-media`: image column — `flex: 0 0 100px` (fixed 100px wide), `align-self: stretch`
  so it fills the full card height. Inner `img`: width 100px, height 100%, object-fit cover
  (crops to a square that fills the column), display block.
- `.partner-card-body`: `flex: 1`, padding 14px 16px.
- `.partner-card-title`: 13px / weight 600 / #1E1E1E, margin-bottom 10px, padding-bottom 8px,
  border-bottom 0.5px #d4e8c8.

### Card bullets (single-line, no sub-text)
- A simpler inline bullet than the `.sc-bullet` pattern — single-line items only.
- `.partner-bullets`: no list-style, no margin/padding.
- `.partner-bullet`: position relative, padding-left 14px, 12px / #555 / line-height 1.5,
  margin-bottom 6px (last-child 0). `::before` is a 6px lime (#A8D44F) dot at top 5px / left 0
  (≈8px visual gap to the text).
