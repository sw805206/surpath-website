...
#### Phone country code + number row (dark pref band) — `.phone-cc` (2026-06-13)
- `.phone-row`: flex, gap 8px, align-items center.
- `.phone-cc`: country-code select — `flex: 0 0 140px`; carries `.text-input` for its appearance + 13px (the `flex: 0 0 140px` overrides the default 100% width). Supersedes the former `.phone-row select.text-input { min-width: 120px }` element rule.
- Phone number `input.text-input`: `flex: 1`, 13px.
...
- `.tz-mismatch`: hidden by default (display none), `.show` → display block. 11.5px / italic / #888 (var --color-footnote) / margin-top 5px. **Color changed from lime #A8D44F to grey #888 (2026-06-13)** — it is an inline note, not a lime accent. Non-blocking informational note ("This differs from your detected timezone"), shown below the timezone select only when the user manually picks a timezone differing from the browser-detected one.
...
- **`.dd-option`**: a `<label>` wrapping a hidden checkbox + visible text + the check indicator. Flex row, space-between, padding 10px 14px, 13px / #1E1E1E, cursor pointer. Hover → #F5F9F0 bg. Checked (`:has(input:checked)`) → #F5F9F0 bg.
- **`.dd-option input`** (the real checkbox): visually hidden (clip rect) — it remains the form value and the `:has()` state source.
- **`.dd-check`**: a 14px teal (#4A9E8E) checkmark (✓), right-aligned inside the option; `opacity: 0` by default, `opacity: 1` only when the option is checked (`.dd-option:has(input:checked) .dd-check`).
