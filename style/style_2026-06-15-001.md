...
- Interactive **Leaflet map** with **Carto "dark matter" (`dark_all`)** raster tiles, driven by
  inline JS (replaces the earlier static inline SVG). Tile URL
  `https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png` with `subdomains: 'abcd'` and
  `maxZoom: 19`; attribution credits OpenStreetMap contributors + CARTO. Carto tiles need no API
  key (replaced Stadia "alidade_smooth_dark", which returned 401 Invalid Authentication — PR #53).
