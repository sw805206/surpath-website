...
- Legend bar: background #1e3a45, padding 10px 28px, flex row, flex-wrap, gap 20px
  ...
  - Label: 12px / #c8dde3
  - Labels are single-word type only: "Headquarter", "Office", "Warehouse"
- Tile host is the plural CARTO domain `{s}.basemaps.cartocdn.com` (the singular `basemap.cartocdn.com` does not resolve in production)
...
- Section body splits into 2 columns: ~60% bullets (left) / ~40% whitepaper peek (right)
- Gap 32px, columns align at top (whitepaper peek margin-top reset to 0 inside this layout)
- Mobile (≤768px): stack vertically, bullets first then peek box
