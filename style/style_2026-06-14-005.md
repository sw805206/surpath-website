...
## Partners Page
Style groundwork for a list + carousel Partners layout (not yet wired into any HTML). All CSS
lives in style.css under `/* Partner page */`. Values map to the existing `:root` tokens (noted
inline as hex). This layout is an alternative to the full-bleed band design documented under
**Partner Blocks** below; both sets of patterns coexist in the stylesheet.
...
- `.partners-list-col`: width 30%, `#F5F9F0` bg (var --color-section-sage), position sticky / top 0, padding 24px 20px. No border-right.
- `.partners-carousel-col`: flex 1 (the remaining 70%), padding 24px, `#fff` bg.
...
### Carousel card
- `.partners-card`: 0.5px solid `#d4e8c8` border, 8px radius, overflow hidden.
- `.partners-card-img`: width 100%, height 200px, object-fit cover, display block.
- `.partners-card-body`: padding 20px.
