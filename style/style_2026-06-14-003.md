...
- Two-column wrapper (`.sc-doc-layout`): display flex, align-items: stretch, gap 32px. Left column (`.sc-doc-main-col`: flex 6, flex column, `min-width: 0`) wraps the bullet list (`.sc-bullets`) **plus the CTA beneath it**; right column (`.sc-doc-col`) flex 4. Mobile (≤768px): stack to a single column, gap 24px (`.sc-doc-main-col` and `.sc-doc-col` → flex 1 1 auto)
- Right column (`.sc-doc-col`): flex column, `align-items: stretch`, `justify-content: flex-start`, gap 16px, `min-width: 0` (lets it shrink below the 720px iframe's intrinsic width so overflow clips). Order top→bottom: label → iframe peek. The peek sits **just below the label** (`margin-top: 8px` on `.sc-doc-peek`, added to the column's 16px gap → 24px label→peek), which lands the peek's bottom edge level with the **last bullet's** bottom edge in the left column; any extra stretched height (from `.sc-doc-layout` `align-items: stretch`) falls as empty space below the peek.
...
- Iframe peek box (`.sc-doc-peek`): position relative, height 280px (fixed), overflow hidden, **sage (#F5F9F0, var(--color-section-sage)) bg** (not white), 0.5px #d4e8c8 (card) border, 8px (card) border-radius, cursor pointer. Entire box clickable — inline `onclick` opens whitepaper-sample.html (same tab)
  ...
  - Bottom fade (`.sc-doc-fade`): absolute, full width, height 70px, gradient transparent → **sage (#F5F9F0, var(--color-section-sage))** to match the peek bg; the transparent start is sage-toned (`rgba(245, 249, 240, 0)`) so it blends cleanly without a white ghost. pointer-events none. Same treatment as `.whitepaper-fade`, taller
...
- CTA (`.sc-doc-cta`): a subdued **text link in the left column, directly below the bullet list** (not in the right column, not adjacent to the peek box). Style matches the contact-page Restart link — 12px / #4A9E8E (mid teal) / underline / no background / no border, `margin-top: 12px` for breathing room. Text: "Contact us to get your customized white paper", links to contact.html. (Replaces the former full-width `.btn.btn-dark` CTA; the generic `.btn-dark` token remains defined in the Buttons section but is no longer used in markup.)
...
- **Restart link** (`.topics-restart`): a plain `<span>` (role="button", tabindex 0) on **its own full-width row below the topic buttons** (`grid-column: 1 / -1`), right-aligned (`justify-self: end`) — 12px / #4A9E8E / underline / cursor pointer. It no longer shares the last button row with "General inquiry". Clicking (or Enter/Space) clears all topic-button selections, hides every dynamic question block, and clears all field values inside those blocks. It does **not** reset Zone 1 fields (name, company, email, etc.).
