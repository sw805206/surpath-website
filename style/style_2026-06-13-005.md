...
#### Volume fields (number input + inline unit label)
- `.volume-row`: flex, align-items center, gap 8px — pairs a number input with a unit label rendered **outside** the input box.
- `.volume-row .text-input`: `flex: 0 1 160px; width: auto` (overrides the default 100% width so the unit sits beside it, not below).
- `.volume-unit`: 12.5px / weight 400 / #555 (var --color-text-secondary) / white-space nowrap. Unit text e.g. "loads / month", "packages / month", "pallets / month".
- All three volume inputs are `type="number" min="0" step="1"` (integer only).
...
#### Phone country code + number row (dark pref band)
- `.phone-row`: flex, gap 8px. Country-code `select.text-input`: `flex: 0 0 auto; width: auto; min-width: 120px`; number `input.text-input`: `flex: 1`. Replaces the former single phone input.

#### Timezone field & inline notes (dark pref band)
- The timezone `select` is now **always visible** at the top of the pref band, above the method selector.
- `.field-note`: small inline note (11px / line-height 1.5 / margin-top 4px). Inside the pref band it is colored var(--color-text-on-dark) (#c8dde3). Used for "We'll do our best to reach you during your business hours." (a note, not a `.field-hint`).
- `.tz-mismatch`: hidden by default, `.show` reveals it. 11px / italic / lime #A8D44F (var --color-lime) / margin-top 5px. Non-blocking informational note ("This differs from your detected timezone"), shown only when the user manually picks a timezone differing from the browser-detected one.
...
- Order inside `.pref-band`: intro → timezone field (always visible) → method selector → phone fields (when Phone selected).
