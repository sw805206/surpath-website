...
- Interactive **Leaflet map** with **Stadia "alidade_smooth_dark"** raster tiles, driven by
  inline JS (replaces the earlier static inline SVG). Tile URL
  `https://tiles.stadiamaps.com/tiles/alidade_smooth_dark/{z}/{x}/{y}{r}.png?api_key=…` with
  `maxZoom: 20`; attribution credits Stadia Maps + OpenMapTiles + OpenStreetMap contributors. The
  `?api_key=` query param authenticates the tiles (the literal key lives in services.html, not here).
  History: the map briefly used Carto `dark_all` (PR #53) as a no-key workaround when Stadia
  returned 401 Invalid Authentication; PR #55 restored Stadia with a valid API key.
  Leaflet CSS + JS are loaded from the unpkg CDN; the script fails gracefully (no-op) if Leaflet
  (`L`) is unavailable.
...
## Partners Page
The Partners page (partners.html) uses this list + carousel layout. All CSS lives in style.css
under `/* Partner page */`. Values map to the existing `:root` tokens (noted inline as hex). It
replaced an earlier full-bleed band design that reused the Services-page `.svc-block` pattern
(those classes remain documented under **Service Blocks** above and are still used by services.html).

### Partners banner
- `.partners-banner`: full-bleed dark band, `#1e3a45` bg (var --color-deep), padding 40px 0; content sits in `.container`.
- `.partners-banner-eyebrow`: 11px / weight 500 / uppercase / letter-spacing 0.1em / `#4A9E8E` (var --color-mid-teal) / margin-bottom 10px.
- `.partners-banner-body`: 14px / `#c8dde3` (var --color-text-on-dark) / line-height 1.7.
- **Banner background fix (2026-06-14):** `.partners-banner` was computing transparent because the note comment at the top of the `/* Partner page */` block contained a nested `/* … */` (CSS comments don't nest) — the early `*/` truncated the comment and the parser's error-recovery swallowed the following `.partners-banner` rule. The note comment was corrected; the `var(--color-deep)` background (`#1e3a45`) now applies.

### Section layout (list 30% / carousel 70%)
- `.partners-layout`: flex, align-items stretch, gap 0 (columns sit flush), min-height 480px so the carousel card shows fully.
- `.partners-list-col`: width 30%, `#F5F9F0` bg (var --color-section-sage), position sticky / top 0, padding 24px 20px. No border-right. The left list is the **height reference** — `align-items: stretch` on the layout makes the carousel column match its height.
- `.partners-carousel-col`: flex 1 (the remaining 70%), padding `24px 24px 0` (**no bottom padding** so the stretched card's bottom border reaches the column edge and aligns with the left list column), `#fff` bg. **Flex column** (display flex / flex-direction column) so the card below the controls fills the column height. On mobile (≤768px) padding is overridden to 16px all sides.

### List groups
- `.partners-group-title`: 11px / weight 500 / uppercase / letter-spacing 0.1em / `#4A9E8E` / margin-top 16px / margin-bottom 6px. First group has no top margin (`.partners-group:first-child .partners-group-title`).
- `.partners-firm`: 13px / `#555` (var --color-text-secondary) / line-height 1.8 / padding-left 10px.
- `.partners-firm.active`: `#fff` bg, border-left 2px solid `#A8D44F` (var --color-lime), padding-left 8px (2px less, so text stays aligned), `#1E1E1E` (var --color-text), weight 500.

### Carousel controls
- `.partners-carousel-controls`: flex, gap 8px, margin-bottom 16px.
- `.partners-arrow`: ghost style — transparent bg, 1px solid `#d4e8c8` (var --color-card-border) border, 6px radius, padding 4px 10px, 13px, `#2C4F5C` (var --color-primary), cursor pointer.
- Disabled (`:disabled` / `.disabled`): opacity 0.35, cursor default.

### Carousel card
- `.partners-card`: 0.5px solid `#d4e8c8` border, 8px radius, overflow hidden. **Flex column with `flex: 1`** so it stretches to fill `.partners-carousel-col`, aligning its bottom edge with the left list column (the height reference).
- `.partners-card-img`: width 100%, height 200px (fixed — `flex-shrink: 0` so it doesn't compress in the flex column), object-fit cover, display block. Default object-position is centered; the Sourcing image (flea-market) overrides to `object-position: center bottom` via `.partners-card-img[src*="flea-market"]` (the carousel reuses one `<img>` whose src swaps per card, so the override is keyed to the src).
- `.partners-card-body`: padding 20px, `flex: 1` (takes the remaining height below the fixed image).
- `.partners-card-title`: 16px / weight 500 / `#1E1E1E` / margin-bottom 12px.
- `.partners-card-item`: 13px / `#555` / line-height 1.8.

### Mobile (≤768px)
- `.partners-layout`: flex-direction column.
- `.partners-list-col`: position static, width 100%.
- `.partners-carousel-col`: width 100%, padding 16px.
- Card stacks list col → image → card body (the image stays inside `.partners-card`, so the natural DOM order is preserved).
