...
replaced the earlier full-bleed band design still documented under **Partner Blocks** below —
that band pattern is no longer used by partners.html.
...
- `.partners-carousel-col`: flex 1 (the remaining 70%), padding 24px, `#fff` bg. **Flex column** (display flex / flex-direction column) so the card below the controls stretches to fill the column height (see Carousel card).
...
- `.partners-card-img`: width 100%, height 200px (fixed — `flex-shrink: 0` so it doesn't compress in the flex column), object-fit cover, display block.
...
## Partner Blocks (Partners page — partners.html)
The Partners page (partners.html) reuses the shared nav, hero (standard order: slogan → H1 →
body, plus a lime `.btn .btn-lime` "Contact us" CTA in `.hero-cta`), and footer. Its category
blocks **reuse the `.svc-block` pattern from the Services page** (see Service Blocks) — no
partner-specific CSS. Five full-bleed alternating bands (`.band-white` / `.band-sage`, starting
white), one per category, each a `.svc-block` with the image left (no `--reverse` variant):
`.svc-block-media` (40% / 360px image), `.svc-block-content`, `.svc-block-eyebrow` (number
"01"–"05"), `.svc-block-title` (category name), and a `.svc-bullets` list (one teal-dot bullet
per service item, no sub-text). Mobile rules are inherited from `.svc-block` (image stacks above
content, full width). The `/* Partner page */` section in style.css carries no rules, only a note.
